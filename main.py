...
Gravatar_api_key="1470:gk-LJ2PCrTzHzc8UF8heC0RIraNwu5Jj7EaiI-wmkD6WPJWP-n3zgZHUJ-1NnwX8"
...
def login():
    ...
    if form.validate_on_submit():
        ...
        user=result.scalar()
        if not user:
            flash(message='User is not currently registered.Please register')
            return redirect(url_for('login'))
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash(message='Please login. You are not logged in' )
            return redirect(url_for('login'))
